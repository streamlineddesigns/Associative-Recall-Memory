...
I tried two more tricks I hadn’t gotten the chance to return to. The first was that, I simply used real time learned cluster to policy mappings. Essentially a group of obstacles that mapped to a policy. The policy was symbolic. The agent learned to pass every obstacle. I deleted this as well. The second was that I figured if the Q table approach worked for action probabilities, it would also work for dynamics modeling. Not probabilistic modeling, but deterministic. It achieved some relatively high percentage of accuracy. This agent wasn’t scored on anything but world model prediction. I suspected the following approach generalizes. Especially since residual connections are used and the memories are used as “starting points” instead of predicting future states from scratch. A memory informed world model of this sort would certainly expand upon the cognitive framework a step further. At one point or another, I felt that I was “on to some kind of breakthrough” and again, I deleted this too. That startled feeling was a very strange paradox. In one sense, I felt that I had achieved something incredible. In another, it felt that I was discovering something frightening. I feel that maybe some people could relate to this, but seeing it in real time for the first time, and being the cause, is even more frightening.
...
No Values aren't "action values", they are value estimations  
